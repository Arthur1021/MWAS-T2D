...
if (!file.exists("5_EA_model_selection_validation")) {
  # Create the directory
  dir.create("5_EA_model_selection_validation")
}

finished_files <- Sys.glob('5_EA_model_selection_validation/*best*')
finished_files_df <- data.frame(finished_files)
finished_files_df$CpG <- sub(".*/(cg\\d+)_.*", "\\1", finished_files_df$finished_files)

#read pheno raw data
pheno_true <- fread('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/methylation_m_value.txt', data.table =F)
...
for (chr_id in 1:22){
    ...
    geno_selection <- BEDMatrix(paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/All_selection_chr',chr_id ), simple_names = TRUE)
    ...
    #bim for selection
    bim_selection <- fread(paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/All_selection_chr', chr_id, '.bim'), data.table =F)
    ...
    cov_selection_pca <- fread('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/selection_PCA/combined_indep_PCA.pca.evec', data.table = F, skip = 1)
    ...
    geno_validation <- BEDMatrix(paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/All_validation_chr',chr_id), simple_names = TRUE)
    #impute missing geno with mean value
    ...
    #bim for validation
    bim_validation <- fread(paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/All_validation_chr',chr_id,'.bim'), data.table =F)
    ...
    cov_validation_pca <- fread('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA/validation_PCA/combined_indep_PCA.pca.evec', data.table = F, skip = 1)
    ...
    files <- list.files(paste0('/mnt/lvm_vol_2/txu/projects/mwas/fredhutch/updated.data.202408/WA.updated.analysis.202410/p13.WA.chr', chr_id, '/'), pattern =glob2rx("clean.metal.cg*.model"))
    #get overlap cpg id
    ...
    selection_validation <- function(id){
        ...
        #read model weight
        model <- read.table(paste0('/mnt/lvm_vol_2/txu/projects/mwas/fredhutch/updated.data.202408/WA.updated.analysis.202410/p13.WA.chr', chr_id, '/clean.metal.', id, '.1.tbl.model'), sep = ' ')
        # model <- model[,-1]
        #read model snps
        snps <- read.table(paste0('/mnt/lvm_vol_2/txu/projects/mwas/fredhutch/updated.data.202408/WA.updated.analysis.202410/p13.WA.chr', chr_id, '/clean.metal.', id, '.1.tbl.snps'), sep = ' ')
        ...
        if (nrow(snps) == nrow(model)){
            ...
            write.table(merge_res, paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA_model_selection_validation/', id, '_all_models_R2.txt'), row.names = FALSE, sep = '\t')
            ...
            write.table(validate_res, paste0('/mnt/lvm_vol_2/sliu/project/MESA_methylation_WGS_data/5_EA_model_selection_validation/', id, '_best_models_R2_validation_r2.txt'), row.names = FALSE, quote =FALSE, sep = '\t')
